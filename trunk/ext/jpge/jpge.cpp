...
//      , Dec. 14, 2014: Alex Admiraal: Use std::ostream instead of output_stream.
...
{
  m_all_stream_writes_succeeded = m_all_stream_writes_succeeded && m_pStream->write(reinterpret_cast<const char *>(&i), sizeof(i));
}
...
void jpeg_encoder::flush_output_buffer()
{
  if (m_out_buf_left != JPGE_OUT_BUF_SIZE)
    m_all_stream_writes_succeeded = m_all_stream_writes_succeeded && m_pStream->write(reinterpret_cast<const char *>(m_out_buf), JPGE_OUT_BUF_SIZE - m_out_buf_left);
  m_pOut_buf = m_out_buf;
  m_out_buf_left = JPGE_OUT_BUF_SIZE;
}
...
bool jpeg_encoder::init(std::ostream &stream, int width, int height, int src_channels, const params &comp_params)
{
  deinit();
...
{
  std::ofstream dst_stream(pFilename);
...
bool compress_image_to_jpeg_file_in_memory(std::ostream &stream, int width, int height, int num_channels, const uint8 *pImage_data, const params &comp_params)
{
   jpge::jpeg_encoder dst_image;
   if (!dst_image.init(stream, width, height, num_channels, comp_params))
      return false;
