...
namespace _private {
  ...
  static lxivec_always_inline void repack(Ints<uint8_t, _count> &dst, const Floats<_count> &src)
  {
    ...
    for (; i<_count; i++)
      dst.val[i] = saturate_uint8(int32_t(src.val[i]));
  }
  ...
  static lxivec_always_inline void repack(Ints<int8_t, _count> &dst, const Floats<_count> &src)
  {
    ...
    for (int vi = 0; vi < int(sizeof(src.vec) / sizeof(src.vec[0])); vi++)
    {
      dst.vec[vi] = _mm_packs_pi16(
          _mm_packs_pi32(
              _mm_cvtps_pi32(src.vec[vi * 2    ]),
              _mm_cvtps_pi32(_mm_movehl_ps(src.vec[vi], src.vec[vi * 2    ]))),
          _mm_packs_pi32(
              _mm_cvtps_pi32(src.vec[vi * 2 + 1]),
              _mm_cvtps_pi32(_mm_movehl_ps(src.vec[vi], src.vec[vi * 2 + 1]))));
    }
    ...
    for (; i<_count; i++)
      dst.val[i] = saturate_int8(int32_t(src.val[i]));
  }
  ...
  //      _mm_packs_epi32(
  //          _mm_sub_epi32(_mm_cvtps_epi32(src.vec[vi * 2    ]), _mm_set1_epi32(32768)),
  //          _mm_sub_epi32(_mm_cvtps_epi32(src.vec[vi * 2 + 1]), _mm_set1_epi32(32768))),
  ...
  //  i += sizeof(dst.vec) / sizeof(dst.val[0]);
    ...
    for (; i<_count; i++)
      dst.val[i] = saturate_uint16(int32_t(src.val[i]));
  }
  ...
  static lxivec_always_inline void repack(Ints<int16_t, _count> &dst, const Floats<_count> &src)
  {
    ...
    for (int vi = 0; vi < int(sizeof(src.vec) / sizeof(src.vec[0])); vi++)
    {
      dst.vec[vi] = _mm_packs_pi32(
          _mm_cvtps_pi32(src.vec[vi]),
          _mm_cvtps_pi32(_mm_movehl_ps(src.vec[vi], src.vec[vi])));
    }
    ...
    for (; i<_count; i++)
      dst.val[i] = saturate_int16(int32_t(src.val[i]));
  }
  ...
  static lxivec_always_inline void repack(Ints<int32_t, _count> &dst, const Floats<_count> &src)
  {
    ...
    for (int vi = 0; vi < int(sizeof(src.vec) / sizeof(src.vec[0])); vi++)
    {
      dst.vec[vi * 2    ] = _mm_cvtps_pi32(src.vec[vi]);
      dst.vec[vi * 2 + 1] = _mm_cvtps_pi32(_mm_movehl_ps(src.vec[vi], src.vec[vi]));
    }
