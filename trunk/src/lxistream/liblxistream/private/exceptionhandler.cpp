...
/***************************************************************************
 * Based on exchndl.c by José Fonseca <j_r_fonseca@yahoo.co.uk>            *
 ***************************************************************************/
...
struct ExceptionHandler::Module
{
#ifdef Q_OS_WIN
  HMODULE                       handle;
  HANDLE                        process;
...
void ExceptionHandler::initialize(void)
{
#ifdef Q_OS_WIN
  struct T
  {
    ...
    static LONG WINAPI topLevelExceptionFilter(PEXCEPTION_POINTERS exceptionInfo)
    {
      ...
      if (!recursed)
      {
        ...
        Log::startLogMsg("EXC");
        Log::printf("%s caused exception %s(%08x) at location %08x %s",
                    moduleName,
                    excName,
        ...
        {
          Log::printf("  eax=%08lx ebx=%08lx ecx=%08lx edx=%08lx esi=%08lx edi=%08lx\n",
        ...
        {
          Log::printf("  eip=%08lx esp=%08lx ebp=%08lx iopl=%1lx %s %s %s %s %s %s %s %s %s %s\n",
        ...
        {
          Log::printf("  cs=%04lx  ss=%04lx  ds=%04lx  es=%04lx  fs=%04lx  gs=%04lx\n",
                      context->SegCs, context->SegSs, context->SegDs,
                      context->SegEs, context->SegFs, context->SegGs);
        }

        logStackTrace((void *)context->Eip, (void *)context->Esp, (void *)context->Ebp);
  ...
  if (!::IsDebuggerPresent())
    ::SetUnhandledExceptionFilter(&T::topLevelExceptionFilter);
#endif
}

void ExceptionHandler::logStackTrace(const StackFrame &stackFrame)
{
  logStackTrace(NULL, stackFrame.stackPointer, stackFrame.framePointer);
}

void ExceptionHandler::logStackTrace(void *instructionPointer, void *stackPointer, void *framePointer)
{
#ifdef Q_OS_WIN
                          ...
                          NULL))
  {
    ...
    while (stackFrame.AddrFrame.Offset != 0)
    {
      ...
      if ((module.handle = (HMODULE)getModuleBase((void *)stackFrame.AddrPC.Offset)) != NULL)
      {
        ...
        {
          Log::printf("  %s:%08x", moduleName, stackFrame.AddrPC.Offset);
...
void * ExceptionHandler::getModuleBase(void *address)
{
#ifdef Q_OS_WIN
...
ExceptionHandler::Symbol ExceptionHandler::getSymbolFromAddress(const Module &module, void *address)
{
  Symbol result;
  result.name[0] = 0;
  result.fileName[0] = 0;
...
bool ExceptionHandler::demangleSymbolName(const char *name, char *demangledName, size_t size)
{
