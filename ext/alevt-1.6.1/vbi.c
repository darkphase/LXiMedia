...
int
vbi_fdset_select(struct vbi *vbi, int timeout /*millisec*/)
{
    return fdset_select(vbi->fds, timeout);
}
...
vbi_open(char *vbi_name, struct cache *ca, int fine_tune, int big_buf)
{
    ...
    fdset_init(vbi->fds);
    fdset_add_fd(vbi->fds, vbi->fd, vbi_handler, vbi);
